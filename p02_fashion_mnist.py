...
# # parser for Q5  Chenglin
# parser.add_argument('--q5d', type = float, default = '0.5', metavar = 'Q5 dropout rate', help = 'set the dropout rate for Question 5')

# Michael's code for Q5
parser.add_argument('--dropout', type = float, default=0.5, metavar='DO', 
                    help='dropout rate (default: 0.5)')

# Q14
parser.add_argument('--q14', action = 'store_true', help = 'specify to conduct question 14')
parser.add_argument('--q15', action = 'store_true', help = 'specify to conduct question 15')
...
def prepareDatasetAndLogging(args):
    ...
    training_run_name = args.dataset + '_' + args.name
    ...
    callbacklist = callbacks.CallbackList(
        ...
        #  callbacks.CSVLogger(filename=training_run_dir + '.csv',
        #                     output_on_train_end=output_on_train_end)])
...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        dropout_rate = 0.5 if 'args' not in globals() else parser.parse_args().dropout
...
class P2Q9DropoutNet(nn.Module):
    ...
    def forward(self, x):
        dropout_rate = 0.5 if 'args' not in globals() else parser.parse_args().dropout
        x = self.do1(self.bn1(F.relu(F.max_pool2d(self.conv1(x), 2))))
        x = F.relu(F.max_pool2d(self.conv2(x), 2))
        x = x.view(-1, 320)
        x = F.relu(self.fc1(x))
        x = F.dropout(x, p=dropout_rate, training=self.training)
...
class P2Q10DropoutBatchnormNet(nn.Module):
    ...
    def forward(self, x):
        dropout_rate = 0.5 if 'args' not in globals() else parser.parse_args().dropout
        x = self.bn1(self.do1(F.relu(F.max_pool2d(self.conv1(x), 2))))
        x = F.relu(F.max_pool2d(self.conv2(x), 2))
        x = x.view(-1, 320)
        x = F.relu(self.fc1(x))
        x = F.dropout(x, p=dropout_rate, training=self.training)
...
class P2Q12RemoveLayerNet(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 10, kernel_size=5)
        # the second convolutional layer is removed
...
class P2Q13UltimateNet(nn.Module):
    def __init__(self):
        ...
        # TODO Implement me  Chenglin
        self.conv1 = nn.Conv2d(1, 25, kernel_size = 5, padding=2)
        self.pool = nn.MaxPool2d(2, 2)
        self.conv2 = nn.Conv2d(25, 50, kernel_size = 5, padding=2)
...
def run_experiment(args):
    training_run_name = args.dataset + '_' + args.name
    print('\n'+ 'RUNNING TEST FOR: ' + training_run_name)
    ...
    for epoch in range(1, epochs_to_run + 1):
        ...
        # train for 1 epoch
        total_minibatch_count = train(model, optimizer, train_loader, tensorboard_writer, callbacklist, epoch, total_minibatch_count)
        # validate progress on test dataset
        val_acc = test(model, test_loader, tensorboard_writer,
                       callbacklist, epoch, total_minibatch_count)
        if val_acc >= best_val_acc and val_acc >= 0.92 and args.model == 'P2Q13UltimateNet' and args.q15 == False:
            best_val_acc = val_acc
            torch.save(model.state_dict(), './q13.pt')
